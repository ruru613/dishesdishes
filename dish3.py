...
# --------------------- 用户ID管理 ---------------------
# 从URL参数获取user_id，如不存在则生成新的
url_params = st.experimental_get_query_params()
user_id = url_params.get("user_id", [str(uuid.uuid4())[:8]])[0]  # 截取前8位作为可读ID
...
def load_collaborative_filtering_model():
    ...
    try:
        if Path(RATINGS_FILE).exists():
            data_df = load_all_ratings(all_users=True)  # 使用所有用户数据训练模型
...
def home_page():
    ...
    st.markdown(f"""
    <a href="{st.experimental_get_base_url()}?user_id={st.session_state.user_id}">
