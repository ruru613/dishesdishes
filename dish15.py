...
if 'user_id' not in st.session_state:
    # 使用 st.query_params 获取URL参数（替代旧版本的st.experimental_get_query_params）
...
def load_all_ratings(all_users=False):
    ...
    try:
        ...
        if ratings_file.exists():
            ...
            # 打印调试信息
            st.write("加载的Excel原始数据（共{}条）：".format(len(df)), df)
            if all_users:
                return df
            else:
                # 筛选当前用户数据
                user_df = df[df['user_id'] == st.session_state.user_id]
                st.write("当前用户评分数据（共{}条）：".format(len(user_df)), user_df)
                return user_df
...
def setup_chart_fonts():
    """强制从GitHub仓库路径加载字体（绝对路径）"""
    try:
        # 1. 获取当前文件路径（关键：确保和字体文件同仓库）
        current_path = pathlib.Path(__file__).parent  
        font_path = current_path / "SourceHanSansSC-Regular.otf"  # 直接拼接
        
        # 2. 验证字体文件存在（仓库已上传，理论应存在）
        if not font_path.exists():
            st.error("❌ 字体文件丢失！请检查仓库是否包含该文件")
            return
            
        # 3. 注册字体到Matplotlib（强制用绝对路径）
        fm.fontManager.addfont(str(font_path.resolve()))  # resolve() 确保绝对路径
        plt.rcParams["font.sans-serif"] = "Source Han Sans SC"  
        plt.rcParams['axes.unicode_minus'] = False  
        
        # 保留成功提示，若不想显示可删除
        #st.success("✅ 字体加载成功！已使用仓库中的 SourceHanSansSC-Regular.otf")
...
def save_rating_safely(dish_id, rating):
    ...
    try:
        ...
        st.write("加载的所有用户原始数据（共{}条）：".format(len(all_data)), all_data)
        
        if not all_data.empty:
            # 筛选当前用户的旧评分
            user_old_ratings = all_data[all_data['user_id'] == st.session_state.user_id]
            st.write("当前用户的旧评分（共{}条）：".format(len(user_old_ratings)), user_old_ratings)
            
            # 移除当前用户的旧评分
            all_data = all_data[all_data['user_id'] != st.session_state.user_id]
            st.write("移除当前用户旧数据后：", all_data)
        ...
        # 去重：同一用户+菜品，保留最新（第一条）
        st.write("合并后的全部数据（共{}条）：".format(len(combined_data)), combined_data)
        combined_data = combined_data.drop_duplicates(subset=['user_id', 'dish_id'], keep='first')
        st.write("去重后的数据（共{}条）：".format(len(combined_data)), combined_data)
        ...
        # 显示保存成功的评分
        st.write("成功保存评分：", new_rating)
        st.write("当前用户全部评分（共{}条）：".format(len(user_ratings)), user_ratings)
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("正在识别菜品..."):
            try:
                ...
                # 修复：正确传入model参数
                pred_dish, confidence, probs = predict_dish(img, model)
                ...
                st.subheader("识别概率分布")
                # 调试：显示当前字体配置
                st.write("当前字体配置:", plt.rcParams["font.sans-serif"])
...
def rating_statistics_page():
    ...
    # 直接从文件加载当前用户评分（替代从会话状态取）
    ratings_df = load_all_ratings(all_users=False)
    ...
    st.write("评分分布统计：", rating_counts)
    
    # 绘制图表
    fig, ax = plt.subplots(figsize=(10, 5))
    bars = ax.bar(rating_counts.index, rating_counts.values, color='tomato')
    ax.set_xlabel('评分（星）')
    ax.set_ylabel('次数')
    ax.set_title('您的评分分布')
    ...
    if 'dish_id' in ratings_df.columns and 'dish_name' in dishes_df.columns:
        # 按菜品分组计算平均分
        dish_ratings = ratings_df.groupby('dish_id')['rating'].agg(['mean', 'count'])
        dish_ratings = dish_ratings.rename(columns={'mean': '平均评分', 'count': '评价次数'})
        dish_ratings = dish_ratings.sort_values('平均评分', ascending=False)
        
        # 合并菜品名称
        dish_ratings = pd.merge(
            dish_ratings,
            dishes_df[['dish_id', 'dish_name']],
            on='dish_id'
        )
        
        # 显示结果
        for i, row in dish_ratings.head(5).iterrows():
            st.markdown(f"- {row['dish_name']}: {row['平均评分']:.2f}星 (评价 {row['评价次数']} 次)")
    else:
        st.warning("无法显示最喜欢的菜品，数据格式不正确")
...
def test_page():
    ...
    if test_img and st.button("运行测试"):
        try:
            ...
            # 修复：正确传入model参数
            pred, conf, _ = predict_dish(img, model)
