...
def save_rating_safely(dish_id, rating):
    ...
    try:
        ...
        # 只移除当前用户对同一菜品的旧评分（保留不同菜品的评分）
        if not all_data.empty:
            # 筛选当前用户对同一菜品的旧评分
            same_dish_ratings = all_data[(all_data['user_id'] == st.session_state.user_id) & 
                                        (all_data['dish_id'] == dish_id)]
            
            if not same_dish_ratings.empty:
                st.write(f"发现用户对菜品{dish_id}的旧评分，将被更新")
        ...
        st.success(f"成功保存评分！您给菜品{dish_id}打了{rating}星")
        return True, "评分保存成功"
...
def rating_statistics_page():
    ...
    # 美化表格：添加评分星星显示
    if 'rating' in ratings_df.columns and 'dish_id' in ratings_df.columns:
        # 合并菜品名称
        ratings_with_names = pd.merge(
            ratings_df,
        ...
        # 创建星星显示列
        ratings_with_names['评分'] = ratings_with_names['rating'].apply(
            lambda x: f"{x} {'⭐' * int(x)}"
        )
        
        # 重命名和排序列
        display_df = ratings_with_names[['dish_name', '评分', 'timestamp']]
        ...
        # 格式化时间
        display_df['评分时间'] = display_df['评分时间'].dt.strftime('%Y-%m-%d %H:%M:%S')
        ...
        st.metric("平均评分", f"{avg_rating:.2f}星", f"共 {len(ratings_df)} 条评分")
