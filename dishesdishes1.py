...
import pickle
import asyncio
from matplotlib import font_manager as fm  # 导入字体管理模块
...
# --------------------- 字体配置修改开始 ---------------------
# 定义字体文件路径（字体文件和当前代码文件同一目录）
# 注意：这里换成你实际的字体文件名
font_path = pathlib.Path(__file__).parent / "SourceHanSansSC-Regular.otf"  

try:
    if font_path.exists():
        # 注册字体到 Matplotlib
        fm.fontManager.addfont(str(font_path))
        # 设置 Matplotlib 使用该字体（关键：用字体实际名称）
        plt.rcParams["font.family"] = ["Source Han Sans SC", "sans-serif"]  
        st.success("已加载自定义中文字体：Source Han Sans SC")
        
        # 注入 CSS 让 Streamlit 页面文字也用该字体（可选但推荐）
        ...
        # 备选方案：使用系统/Matplotlib 内置字体
        plt.rcParams["font.family"] = ["SimHei", "sans-serif"]  
        st.warning("未找到自定义字体文件，使用备选中文字体：SimHei")
        
except Exception as e:
    st.error(f"字体配置失败: {e}，使用默认字体")
